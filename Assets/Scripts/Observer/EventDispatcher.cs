	...
	public class EventDispatcher : MonoBehaviour
	{
		...
		public static EventDispatcher Instance
		{
			get
			{
				...
				if (s_instance == null)
				{
					...
					MyDebug.Log("Create singleton : {0}", singletonObject.name);
				}
				return s_instance;
			}
			private set { }
		...
		void Awake ()
		{
			...
			if (s_instance != null && s_instance.GetInstanceID() != this.GetInstanceID())
			{
				...
				MyDebug.Log("An instance of EventDispatcher already exist : <{1}>, So destroy this instance : <{2}>!!", s_instance.name, name);
				Destroy(gameObject);
			}
			else
			{
		...
		{
			// checking params
			MyDebug.Assert(callback != null, "AddListener, event {0}, callback = null !!", eventID.ToString());
			MyDebug.Assert(eventID != EventID.None, "RegisterListener, event = None !!");
		...
		public void PostEvent (EventID eventID, object param = null)
		{
			if (!_listeners.ContainsKey(eventID))
			{
				MyDebug.Log("No listeners for this event : {0}", eventID);
				return;
			}
			...
			{
				MyDebug.Log("PostEvent {0}, but no listener remain, Remove this key", eventID);
		...
		{
			// checking params
			MyDebug.Assert(callback != null, "RemoveListener, event {0}, callback = null !!", eventID.ToString());
			MyDebug.Assert(eventID != EventID.None, "AddListener, event = None !!");
			...
			{
				MyDebug.Warning(false, "RemoveListener, not found key : " + eventID);
			}
		}
