...
public class PunGridV2 : MonoBehaviour, IOnEventCallback
{
    public int columns = 7, rows = 7;
    [SerializeField] PunGridSquareV2 gridSquare;
    // [SerializeField] MyGridSquareSoul gridSquareSoul;
    float cellSize;
    Vector2 _startPos;
    bool[,] _visited;
    PunGridSquareV2[,] _gridSquares;
    ...
    {
        cellSize = .77f;
        _startPos.x = -.86f * cellSize * (columns / 2f - 1) - .86f * cellSize;
    ...
    {
        _gridSquares = new PunGridSquareV2[columns, rows];
        for (int i = 0; i < columns; i++)
        {
            for (int j = 0; j < rows; j++)
            {
                ...
                var posXOffset = i * cellSize * .86f;
                var posYOffset = j * cellSize * 1.015f;
                if (j % 2 == 1) posXOffset += .86f * cellSize / 2;
                posYOffset += (rows / 2 - j) * .234f * cellSize;
                _gridSquares[i, j].transform.position
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        if (eventCode == PunEvents.OnShapeDroppedEventCode)
        {
            object[] data = (object[])photonEvent.CustomData;
            // MyDebug.Log("received OnShapeDroppedEventCode from player {0} with shapeSize {1}",
            //     data[0], data[1]);
            CheckIfShapeCanBePlaced((int) data[0], (int) data[1]);
        }
    }

    void CheckIfShapeCanBePlaced(int actor, int shapeSize)
    {
        ...
        if (_hoveringSquare.Count == shapeSize)
        {
            ...
            //TO DO: raise an event
            if(PhotonNetwork.IsMasterClient)
                PunEvents.SendFinishDroppingShapeEvent(actor, true);
            //shapePooling.FinishDroppingAboveShape(true);

            //bfs
            // Debug.Log(_hoveringSquare.Count);
            StartCoroutine(UnoccupyAdjacentSquares(actor));
        }
        else
        {
            ...
            //TO DO: raise an event
            if(PhotonNetwork.IsMasterClient)
                PunEvents.SendFinishDroppingShapeEvent(actor, false);
            //shapePooling.FinishDroppingAboveShape(false);
        }
    }

    IEnumerator UnoccupyAdjacentSquares(int actor)
    {
        //Debug.Log("UnoccupyAdjacentSquares");
        ...
        //TO DO: raise an event
        // yield return new WaitForSeconds(1f);
        PunEvents.SendGenerateRandomShapeEvent(actor);
